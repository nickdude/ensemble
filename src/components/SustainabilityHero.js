...
export default function SustainabilityHero({ theme, data = sustainabilityData }) {
  const { sustainabilityHero } = data;
  return (
    <section className={`w-full ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'} flex flex-col items-center justify-center py-10 px-4 md:py-20 md:px-16`}>
        ...
        <div className="relative w-full h-[75vh] rounded-xl overflow-hidden mt-16">
            <Image
                ...
                alt={`${sustainabilityHero.title} Hero Image`}
                fill
