...
export default function JourneyTo2029({ theme, data = sustainabilityData }) {
  ...
  return (
    <section className={`px-4 md:px-16 py-10 md:py-20 ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'} flex flex-col md:flex-row w-full gap-6 md:gap-0`}>
            ...
            <div className="w-full md:w-[60%] flex flex-col items-left justify-center order-2 md:order-1">
              <h2 className="font-poppins font-semibold text-3xl md:text-5xl md:px-32 mb-6 md:mb-0">{journeyTo2029.title}</h2>
              <Timeline theme={theme} items={journeyTo2029.timeline}/>
            </div>
    </section>
  );
}
